...
app.post('/request-material', (req, res) => {
    ...
    connection.query('SELECT email FROM login WHERE userId = ?', userId, (error, results) => {
        if (error) {
...
app.post('/add-supplier', (req, res) => {
    const { name, contact_person, contact_number, address, status, created_date, created_by } = req.body;

    // Check if all required fields are present
    if (!name || !contact_person || !contact_number || !address || !status || !created_date || !created_by) {
    ...
    // Execute the query
    connection.query(query, [name, contact_person, contact_number, address, status, created_date, created_by], (error, results) => {
        if (error) {
...
app.post('/api/add-record', (req, res) => {
    const { po_code, supplier_id, item_id, quantity, status, created_by, created_at, updated_at } = req.body;
  
    // Check if all required fields are present
    if (!po_code || !supplier_id || !item_id || !quantity || !status || !created_by || !created_at || !updated_at) {
      res.status(400).json({ error: 'Missing required fields' });
      return;
    }
    ...
    // Execute the query
    connection.query(query, [po_code, supplier_id, item_id, quantity, status, created_by, created_at, updated_at], (error, results) => {
      if (error) {
        console.error('Error executing query:', error);
        res.status(500).json({ error: 'Internal server error' });
        return;
...
app.post('/api/add-item', (req, res) => {
  ...
  // Execute the query
  connection.query(query, [item_id, item_name, added_date, supplier_id], (error, results) => {
    if (error) {
      console.error('Error executing query:', error);
      res.status(500).json({ error: 'Internal server error' });
      return;
